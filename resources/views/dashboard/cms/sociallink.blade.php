@section('pageheader', 'Social Links')
@extends('layouts.app')
...
@section('content')
    <section class="content-header">
        <h1>
            Content Management
            <small>Manage Social Links</small>
        </h1>
        <ol class="breadcrumb">
        ...
        <div class="box">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    @if(Myhelper::can('add_sociallink'))
    ...
    <div class="modal fade-in" id="sociallinkmodal">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <form action="{{route('dashboard.cms.submitcms')}}" method="POST" id="sociallinkform" enctype="multipart/form-data">
                    @csrf
...
@push('script')
    <script>
        $('#my-datatable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{route('dashboard.fetchdata', ['type' => 'sociallinks'])}}",
                type: "POST",
                data:function( d )
                {
                    d._token = '{{csrf_token()}}';
        ...
        function edit(id){
            Pace.track(function(){
                $.ajax({
                    url: "{{route('dashboard.fetchdata', ['type' => 'sociallinks', 'fetch' => 'single'])}}" + "/" + id,
                    data: {'token':'{{csrf_token()}}'},
                    success: function(data){
                        var result = data.result;
                        $('#sociallinkmodal').find('.modal-title').text('Edit Role');
                        $('#sociallinkmodal').find('[name=id]').val(id);
                        $('#sociallinkmodal').find('[name=operation]').val('sociallinkedit');
                        $('#sociallinkmodal').find('[name=tittle]').val(result.tittle);
                        $('#sociallinkmodal').find('[name=link]').val(result.link);
        ...
        function add(){
            $('#sociallinkmodal').find('.modal-title').text('Add New Social Link');
            $('#sociallinkmodal').find('[name=id]').val('');
            $('#sociallinkmodal').find('[name=operation]').val('sociallinknew');
            $('#sociallinkmodal').modal('show');
        }
        ...
        function changeAction(id){
            ...
            Pace.track(function(){
                $.ajax({
                    ...
                    method: "POST",
                    data: {'_token':'{{csrf_token()}}','operation':'sociallinkchangeaction','id':id},
                    success: function(data){
                        $('#my-datatable').dataTable().api().ajax.reload();
                    }, error: function(errors){
                        showErrors(errors);
                ...
                if (willDelete) {
                    Pace.track(function(){
                        $.ajax({
                            url: "{{route('dashboard.cms.submitcms')}}",
                            method: "POST",
                            data: {'_token':'{{csrf_token()}}','operation':'sociallinkdelete','id':id},
                            success: function(data){
                                $('#my-datatable').dataTable().api().ajax.reload();
                            }, error: function(errors){
                                showErrors(errors);
